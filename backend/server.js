...
app.post("/get_is_send_follow", (req, res) => {
  ...
  db.query(sql, (err, data) => {
    if (err || data.length === 0) return res.status(201).json("no reqquest");
    if (data.length > 0) res.status(200).json(data[0]);
  });
});
app.post("/set_new_follow", async (req, res) => {
  ...
  let followers;
  let following;
  let allfollowers_recived_user = "";
  let allfollowing_recived_user = "";
  let allfollowers_request_user = "";
  let allfollowing_request_user = "";
  await axios
    .post(`http://${ip}:8001/get_userdata`, {
      user_id: recived_user_id,
    })
    .then((response) => {
      if (response.status === 200) {
        followers = response.data.followers + 1;
        allfollowers_recived_user =
          response.data.followers_id + `${request_user_id},`;
        allfollowing_recived_user = response.data.following_id;
      }
    })
    .catch((err) => console.log(err));
  await axios
    ...
    .then((response) => {
      if (response.status === 200) {
        following = response.data.following + 1;
        allfollowers_request_user = response.data.followers_id;
        allfollowing_request_user =
          response.data.following_id + `${recived_user_id},`;
      }
    })
    .catch((err) => console.log(err));

  let sql_user1 = `UPDATE user SET followers=${followers},followers_id='${allfollowers_recived_user}' WHERE user_id=${recived_user_id}`;
  let sql_user2 = `UPDATE user SET following=${following},following_id='${allfollowing_request_user}' WHERE user_id=${request_user_id}`;
    ...
    if (err) return res.status(201).json("set data faild");
      ...
      db.query(sql_user2, (err, data) => {
        if (err) return res.status(201).json("set data faild");
          ...
          if (
            allfollowers_recived_user.includes(`,${request_user_id},`) &&
            allfollowing_recived_user.includes(`,${request_user_id},`)
          ) {
            const date = date_and_time()[0];
            const room = `${request_user_id}-${recived_user_id}`;
            const sql_creact_room = `INSERT INTO chat(room,user_1_id,user_2_id,date) VALUES ('${room}',${recived_user_id},${request_user_id},'${date}')`;
...
app.post("/remove_follow", async (req, res) => {
  ...
  let allfollowers = "";
  let allfollowing = "";
  const regex_followers = new RegExp("," + follow_user_id + ",", "g");
  const regex_following = new RegExp("," + user_id + ",", "g");
  ...
  await axios
    ...
    .then((response) => {
      if (response.status === 200) {
        followers = response.data.followers - 1;
        allfollowers = response.data.followers_id.replace(regex_followers, ",");
      }
    })
    .catch((err) => console.log(err));
  await axios
    ...
    .then((response) => {
      if (response.status === 200) {
        following = response.data.following - 1;
        allfollowing = response.data.following_id.replace(regex_following, ",");
      }
    })
    .catch((err) => console.log(err));

  let sql_user1 = `UPDATE user SET followers=${followers},followers_id='${allfollowers}' WHERE user_id=${user_id}`;
  let sql_user2 = `UPDATE user SET following=${following},following_id='${allfollowing}' WHERE user_id=${follow_user_id}`;
  db.query(sql_user1, (err, data) => {
    if (err) return res.status(201).json("set data faild");
    else {
      db.query(sql_user2, (err, data) => {
        if (err) return res.status(201).json("set data faild");
          ...
          const delet_chat_sql = `DELETE FROM chat WHERE room='${user_id}-${follow_user_id}' OR room='${follow_user_id}-${user_id}'`;
          db.query(delet_chat_sql, (err, data) => {
            if (err) return res.status(201).json("set data faild");
          });
          res.status(200).json("success");
        }
